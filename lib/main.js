...
fp.port.on("msg", function(msg) {
  ...
  if (msg === "Relay: connecting.") {
    ...
    proxyPanel.port.emit("status-update", "connecting");
  }
  else if (msg === "Relay: closed.") {
    connactive--;
    if (connactive === 0) {
      proxyBtn.contentURL = data.url("inactive.png");
      proxyPanel.port.emit("status-update", "waiting");
    }
  }
  else if (msg === "Relay: connected.") {
    conntotal++;
    proxyPanel.port.emit("connections-update", conntotal);
  } else if (msg === "Dying.") {
    proxyPanel.port.emit("status-update", "error");
    proxyBtn.contentURL = data.url("error.png");
  } else if (msg.indexOf("Starting") === 0) {
    proxyPanel.port.emit("status-update", "starting");
  }
});
